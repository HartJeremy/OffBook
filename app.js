...
const $=id=>document.getElementById(id); const LS_KEY='offbook.v52'; const LEGACY_KEYS=['offbook.v51','offbook.v50','offbook.v49','offbook.v47','offbook.v46','offbook.v45.multi','screenplayTrainer.v44.multi','screenplayTrainer.v43','screenplayTrainer.v42'];
...
async function extractPdfText(file){
  ...
  for(let pageNum=1;pageNum<=pdf.numPages;pageNum++){
    ...
    const pageText=rows.map(row=>row.items.sort((a,b)=>a.x-b.x).map(i=>i.text).join(' ').replace(/\s+/g,' ').trim()).filter(Boolean).join('
');
    pages.push(pageText);
  }
  return pages.join('
