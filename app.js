...
async function loadScriptOnce(src){
  return new Promise((resolve,reject)=>{
    ...
    script.onload=()=>window.pdfjsLib?resolve(window.pdfjsLib):reject(new Error('PDF reader loaded but did not initialize.'));
    script.onerror=()=>reject(new Error('PDF reader failed from '+src));
...
async function loadPdfJs(){
  ...
  window.__offbookPdfJsPromise=(async()=>{
    let lastErr=null;
    for(const src of sources){
      try{
        setFileReadStatus('Loading PDF reader...');
        const lib=await loadScriptOnce(src);
        if(lib) return lib;
      }catch(err){lastErr=err; console.warn(err);}
    }
    throw new Error('PDF reader could not load. Check internet connection, content blockers, or try text import. Last error: '+(lastErr&&lastErr.message?lastErr.message:'unknown'));
  })();
...
async function extractPdfText(file){
  ...
  // Disable the PDF.js worker. This is slower, but it avoids GitHub Pages/CDN/iOS Safari worker issues.
  if(pdfjsLib.GlobalWorkerOptions){pdfjsLib.GlobalWorkerOptions.workerSrc='';}
  const data=await readFileAsArrayBuffer(file);
  setFileReadStatus('Opening PDF...');
  const loadingTask=pdfjsLib.getDocument({data:new Uint8Array(data), disableWorker:true, useWorkerFetch:false, isEvalSupported:false});
...
async function importSelectedFile(){
  ...
  try{
    ...
    if(name.endsWith('.pdf')||file.type==='application/pdf'){
      ...
      setFileReadStatus(`PDF imported: ${text.trim().length.toLocaleString()} characters extracted. Review detected characters before rehearsing.`, 'ok');
    }else{
